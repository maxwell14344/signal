...
import { getAllCategories, getCategoryToolCount } from "@/lib/tools";
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {categories.map((cat) => (
          <Link
            ...
            className="group flex flex-col gap-4 rounded-2xl border border-border bg-surface p-6 transition hover:border-accent/40 hover:bg-surface-2"
          >
            ...
            <p className="mt-auto text-xs text-muted">
              {getCategoryToolCount(cat.slug)} tools
            </p>
          </Link>
        ))}
      </div>
